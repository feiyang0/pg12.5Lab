	...
	for (;;)
	{
		...
		switch (node->hj_JoinState)
		{
			...
			case HJ_NEED_NEW_INNER:
                ...
                //求出当前取出的内部元组在外部hash表中的hash值，因为保留为null的元组没有意义，所以参数设置为false
				if(ExecHashGetHashValue(outerHashtable, econtext,innerHashNode->hashkeys,true,false,&hashvalue)){
 			...
 			case HJ_SCAN_OUTER_BUCKET:
				...
				if (!(node, econtext)) // false为探测外部表，true为探测内部hash表
				{
